...
# Студент: Maximenco Stanislav, I2302
# Дата выполнения: 21.02.2026
# Задача
Необходимо спроектировать и развернуть VPC в AWS с разделением на публичные и приватные подсети, подключением к интернету через Internet Gateway и NAT Gateway, настройкой таблиц маршрутизации и правил безопасности для изоляции и взаимодействия компонентов инфраструктуры.

# Цель
Освоить базовые принципы построения облачной сети AWS и развертывания многослойной архитектуры приложений.
...
Прошу прощения, забыл переименовать `bastion-sg-kXX` под свой номер, так как пересоздавал её. Вместо `XX` должно быть 06.

## Шаг 9. Проверка работы
Вбиваю публичный айпи web-server и вижу страницу php:

![image](screenshots/Screenshot_23.png)

Подключаюсь к `bastion-host` по SSH:

![image](screenshots/Screenshot_24.png)

Проверяю подключение к интернету с `bastion-host` выполнив `ping`:

![image](screenshots/Screenshot_25.png)

С `bastion-host` пробую подключиться к `db-server`:

Столкнулся с проблемой. Не удается подключиться к инстансу базы данных. Есть ощущение, что по тому как сейчас настроены Security groups, не предоставляется возможности подключиться к базе данных с bastion. Возможно, я что-то не учел и это моя ошибка

## Завершение работы
Удаляю все ресурсы, созданные во время выполнения лабораторной работы.
...
# Вывод
В результате работы была создана функционирующая `VPC`-инфраструктура с изолированными подсетями и маршрутами. Настроены правила безопасности, обеспечивающие доступ в интернет только через `NAT Gateway` и защищенный доступ через `bastion-host`. Веб-сервер успешно взаимодействует с базой данных, а приватные ресурсы остаются недоступными извне.